...
def choose_user(users):
    """
    Handles user selection or creation.
    If no users exist, we force creating one.
    Otherwise, let them pick an existing profile or create a new one.
    """

    def save_user_id_to_file(user_id: str):
        """
        Save the generated user_id locally so the user has a 'secret key'.
        This is written to the Desktop to make it easy to find.
        """
    ...
    # If no users exist yet, we must create one
    if not users:
        ...
        save_user_id_to_file(new_user.user_id)  # Save secret key once
        return new_user
    ...
    if choice == "1":
        # They must know their secret ID to login
        ...
        # Same flow as before, but user decided to create new
        name = input("Name: ").strip()
        group_size = int(input("Group size: ").strip())
        preferred_env = input("Preferred environment: ").strip().lower()
        budget_min = float(input("Budget min: ").strip())
...
def edit_current_user(user: User, users: list):
    """
    Allows the current user to update their profile interactively.
    Skips any field they leave blank.
    """
...
def listings_menu_for_user(user, listings, users):
    """
    Main interactive loop for user-specific actions.
    Handles viewing, filtering, sorting, favorites, recommendations,
    synthetic listings generation, and bookings.
    """
    ...
    while True:
        ...
        if choice == "1":
            ...
            # Convert string inputs to numbers if valid
            min_price = (
            ...
            # Call LLM to generate synthetic listings
            prompt = input(
            ...
            try:
                ...
                # Reload the merged dataset into memory
...
def main():
    """
    Entry point: load data, then drop into menu.
    """
    users = load_users()
